...
def setup():
    global args, workdir
    programs = ['ruby', 'git', 'make', 'wget', 'curl', 'lsb-release']
    if args.kvm:
        programs += ['apt-cacher-ng', 'python-vm-builder', 'qemu-kvm', 'qemu-utils']
    elif args.docker:
        programs += ['apt-cacher-ng']
    ...
    if not os.path.isdir('tipcoin'):
        subprocess.check_call(['git', 'clone', 'https://github.com/tipcoin-dev/tipcoin.git'])
...
def build():
    ...
    os.makedirs('tipcoin-binaries/' + args.version, exist_ok=True)
    print('\nBuilding Dependencies\n')
    os.chdir('gitian-builder')
    os.makedirs('inputs', exist_ok=True)

    subprocess.check_call(['wget', '-O', 'inputs/osslsigncode-2.0.tar.gz', 'https://github.com/decryp2kanon/apple_sdk/releases/download/200923/osslsigncode-2.0.tar.gz'])
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/tipcoin-dev/tipcoin', help='Specify the URL of the repository. Default is %(default)s')
    ...
    if args.pull:
        ...
        os.chdir('../gitian-builder/inputs/tipcoin')
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
